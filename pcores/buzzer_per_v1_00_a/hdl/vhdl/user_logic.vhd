...
-- Date:              Sat Jun 18 21:39:29 2016 (by Create and Import Peripheral Wizard)
   ...
   process (Bus2IP_Clk)
	begin
		if (Bus2IP_Resetn='0') then
			...
			my_timer_irq <= next_my_timer_irq;
...
--state machine
process(current_s, slv_reg2, slv_reg1, b2ip_value, timer_value, slv_reg0, timer_reset_sinc)
	...
	next_output_value<=output_value;
		...
		when upcount =>
			if (b2ip_value=99) then
				...
				if(timer_value=slv_reg1) then
					next_output_value<='1';
					next_timer_value<=timer_value-1;
				end if;
				if(timer_value=slv_reg0) then
					next_s<=downcount;
				else
					next_timer_value<=timer_value+1;
				end if;
  ...
  process(current_s2, slv_reg2, slv_reg3, b2ip_value, timer2_value)
